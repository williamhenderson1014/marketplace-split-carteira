...
export const etapas = [
  ...
  },
  {
    ...
    hora: '19:05',
    nota: 'o valor entra em custódia no gateway, não na conta da AinFast',
  },
  {
    id: 'aceito',
    curto: 'Loja aceitou',
...
export function contas(itens) {
  ...
  const taxaAinfast = Math.round(subtotal * comissao * 100) / 100
  const aoVendedor = Math.round((subtotal - taxaAinfast) * 100) / 100
  const aoMotorista = entrega
  return { linhas, subtotal, entrega, total, taxaAinfast, aoVendedor, aoMotorista }
}
